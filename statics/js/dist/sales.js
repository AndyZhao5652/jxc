			...
			$("#grid").jqGrid({
				...
				afterSaveCell: function(t, i, a, r, n) {
					...
					case "qty":
						...
						if ($.isNumeric(h)) if ($.isNumeric(f)) var m = a * h * f / 100,
							s = a * h - m,
					...
					case "price":
						...
						if ($.isNumeric(g)) if ($.isNumeric(f)) var m = a * g * f / 100,
							s = a * g - m,
							p = $("#grid").jqGrid("setRowData", t, {
								deduction: m,
								amount: s
					...
					case "amount":
						...
						if ($.isNumeric(a)) {
							...
							if ($.isNumeric(y) && $.isNumeric(h)) {
								var v = y * h,
									f = v ? (100 * m / v).toFixed(amountPlaces) : 0;
								$("#grid").jqGrid("setRowData", t, {
									discountRate: f
								})
							}
