...
class Data_model extends CI_Model{
	...
	public function invsa_info($where='',$order='') {
        ...
        //trans info
	public function trans_info($where='',$order='') {
	    $where = $where ? 'where (1=1) '.$where : '';
	    $sql = 'select a.* from '.TRANS_INFO.' as a 
				'.$where.' 
				'.$order.'
				';
		return $this->cache_model->load_sql(INVSA_INFO,$sql,2);		
	...
	public function cus_contract($where='') {
	    $where = $where ? 'where (1=1) '.$where : '';
	    $sql = 'SELECT 
                        name as customername,
                        number as customernumber,
                        address,
                        payterm,
                        packing,
                        shipfrom,
                        ...
                        phone
                    FROM '.CONTACT.' '.$where;
		return $this->cache_model->load_sql(CONTACT,$sql,2);		
	}
