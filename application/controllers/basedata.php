...
//		$type   = intval($this->input->get('type',TRUE));
		...
		$offset = $rows * ($page-1);
		$data['data']['page']      = $page;                                                      //当前页
		$data['data']['records']   = $this->cache_model->load_total(TRANS,'(1=1) '.$where.'');     //总条数
		$data['data']['total']     = ceil($data['data']['records']/$rows);                       //总分页数
		$list = $this->cache_model->load_data(TRANS, '(1=1) '.$where.'');   
		foreach ($list as $arr=>$row) {
		    $v[$arr]['id']           = intval($row['id']);
			$v[$arr]['shipno']    = $row['shipno'];
			$v[$arr]['contractno']       = $row['contractno'];
			$v[$arr]['shiptype']         = $row['shiptype'];
			$v[$arr]['shipdate']    = $row['shipdate'];
			$v[$arr]['shipto']       = $row['shipto'];
			$v[$arr]['decinvoiceno']  = $row['decinvoiceno'];
			$v[$arr]['lwh']       = $row['lwh'];
			$v[$arr]['weight']      = $row['weight'];
			$v[$arr]['qty'] = $row['qty'];
			$v[$arr]['shipprice'] = $row['shipprice'];
                        $v[$arr]['customer']       = $row['customer'];
			$v[$arr]['invoiceno']      = $row['invoiceno'];
			$v[$arr]['esarrdate'] = $row['esarrdate'];
                        $v[$arr]['rearrdate'] = $row['rearrdate'];
			$v[$arr]['description'] = $row['description'];

		}
		$data['data']['rows']   = is_array($v) ? $v : '';
		$data['data']['totalsize']  = $this->cache_model->load_total(TRANS,'');  
		die(json_encode($data));
	}
	...
	public function contact_query() {
        ...
        //运输单查询接口
	public function trans_query() {
	    $id   = intval($this->input->post('id',TRUE));
//		$type = intval($this->input->get('type',TRUE));
	    $data = $this->cache_model->load_one(TRANS,'(id='.$id.')');
		 
		if (count($data)>0) {
			$info['id']          = intval($data['id']);
			$info['shipno']   = $data['shipno'];
			$info['contractno']      = $data['contractno'];
                        $info['shiptype']      = $data['shiptype'];
                        $info['shipdate']      = $data['shipdate'];
                        $info['shipto']      = $data['shipto'];
                        $info['decinvoiceno']      = $data['decinvoiceno'];
                        $info['lwh']      = $data['lwh'];
                        $info['weight']      = $data['weight'];
                        $info['qty']      = $data['qty'];
                        $info['shipprice']      = $data['shipprice'];
                        $info['customer']      = $data['customer'];
                        $info['invoiceno']      = $data['invoiceno'];
                        $info['esarrdate']      = $data['esarrdate'];
                        $info['rearrdate']      = $data['rearrdate'];
...
//		    unset($data['linkmans']);
	...
	public function contact_checkname() {
        ...
        //运输单号验证接口
	public function trans_checkno() {
	    $shipno   = $this->input->post('shipno',TRUE);
//		!in_array($shipno,array(1,2)) && die('{"status":-1,"msg":"参数错误"}'); 
                if ($shipno = '') { die('{"status":-1,"msg":"参数错误"}'); }
//		if ($shipno ) {
		    $this->cache_model->load_total(TRANS,' (shipno="'.$shipno.'")') > 0 && die('{"status":-1,"msg":"运输单号重复"}'); 
//		} else {
//		    $this->cache_model->load_total(TRANS,'(type='.$type.') and (name="'.$name.'")') > 0 && die('{"status":-1,"msg":"客户名称重复"}'); 
//		} 
	    die('{"status":200,"msg":"success"}');
	}
