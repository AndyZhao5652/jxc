	...
	public function cus_contract() {
		...
		if ($goodsno) {
			$where .= ' and a.goodsno in('.str_quote($goodsno).')';
                ...
                $data['list'] = $this->data_model->invpu_detail($where,'order by a.id desc');
                $data['contractinfo'] = $this->data_model->cus_contract(' and number like "%'.$contactno.'%"');
                //$data['cusname']=$data['contractinfo'][0]['customername'];
                
		$this->load->view('report/invpu_contract',$data);	
	}
